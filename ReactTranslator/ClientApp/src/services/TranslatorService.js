export default class TranslatorService {
    ...
    executeProgram = async (sourceCode) => {
        const result = await this.getResourcePOST("/Execute", sourceCode);

        return result;
    };

    continueExecuting = async (data) => {
        const result = await this.getResourcePOST("/ContinueExecution", data);
